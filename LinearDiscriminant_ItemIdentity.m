function [trainedClassifier, validationAccuracy] = LinearDiscriminant_ItemIdentity(trainingData,decodedItem)
...
predictorNames = cell(1,nTEST-2);
for m = 1:nTEST-2
    predictorNames{1,m} = strcat('TESTi',num2str(m));
end
predictors = inputTable(:, predictorNames);
% response = inputTable.TESTi156;
response = inputTable(:,strcat('TESTi',num2str(nTEST-2+decodedItem)));
